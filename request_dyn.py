...
def init_dyn_data_download(buoy):
    try:
        url = f"https://www.pmel.noaa.gov/tao/taoweb/disdel_data/cdf/sites/daily/dyn{buoy}_dy.cdf"
        response = requests.get(url)
        if response.status_code == 200:
            with open(f"data/total/dyn{buoy}_dy.cdf", "wb") as f:
...
def  request_dyn_data(year, buoy):
    file_cdf = f'data/total/dyn{buoy}_dy.cdf'
    file_dir = f'data/year/data_year_noaabuoys_dyn_{year}_{buoy}.csv'
    ...
    if not os.path.isfile(file_dir):
        ...
        try:
            ...
            dyn = np.array(data['DYN_13']).T[0][0][0]
            dyn[dyn == 1e+35] = np.nan
...
def request_dyn_historical_data(year, buoy):
    file_cdf = f'data/total/dyn{buoy}_dy.cdf'
    file_dir = f'data/hist/data_hist_noaabuoys_dyn_{year}_{buoy}.csv'
