...
place = 'lima'
for place in  ['lima', 'pisco', 'trujillo', 'chiclayo']:

    data = pd.read_csv(f'vientos {place}.csv', sep=';')
    data['time'] = [i[5:] for i in data['time']]
    años = data['AÑO'].unique()
    #data['u'] = [cws.calc_wind_stresss(uw, 1022) for uw in data['u']]
    #data['v'] = [cws.calc_wind_stresss(vw, 1022) for vw in data['v']]
    ...
    for year in years:
        ...
        plt.suptitle(f'vientos zonales y meridionales en {place[0].upper() + place[1:]}\nAño: {year}')


        ax[0].set_ybound(-20, 20)
        ax[1].set_ybound(-20, 20)

        plt.tight_layout()

        plt.savefig(f'windStressPlots/vientos_u_v_{place}_{year}_anom.png')
        plt.close()