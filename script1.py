...
buoys = [
        '0n165e', '2n165e', '5n165e', '2s165e', '5s165e',
        '0n156e', '2n156e', '5n156e', '2s156e', '5s156e',
        '0n180w', '2n180w', '5n180w', '2s180w', '5s180w',
        '0n170w', '2n170w', '5n170w', '2s170w', '5s170w'
        ]
...
for buoy in buoys:
    ...
    for year in [ '1997', '2015', '2023', '1998', '2016', '2017','2024']:
    #for year in [ '1997']:
        try:
            ...
            data_hist['wsu'] = [cws.calc_wind_stress(u, 1.204) for u in data_hist['winds_u']] 

            data_year['wsu'] = [cws.calc_wind_stress(u, 1.204) for u in data_year['winds_u']]
            data_year['anom_u'] = data_year['winds_u'] - data_hist['winds_u']
            data_year['anom_wsu'] = data_year['wsu'] - data_hist['wsu']

            data_mean = data_year.groupby('month').mean(numeric_only=True)
            data_sum = data_year.groupby('month').sum(numeric_only=True)
            data_count = data_year.groupby('month').count()
            
            data_buoy_expo = pd.DataFrame()
            data_buoy_expo['count'] = data_count['wsu']
            data_buoy_expo['sum'] = data_sum['wsu']
            data_buoy_expo['mean'] = data_mean['wsu']
            data_buoy_expo = data_buoy_expo.reset_index()

            #data_buoy_expo.to_csv(f'data/monthly/{buoy}_{year}.csv', index=False)
            ...
            #plt.show()
            #plt.savefig(f'plots/expo6/plot_ws_{buoy}_{year}.png')
            #plt.close()
            """
